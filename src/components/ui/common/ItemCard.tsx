...
interface ItemCardProps {
  ...
  /** 名前ラベル */
  label: ReactNode;
  /** 画像枠（正方形）のサイズ。持ち物一覧のカードに合わせてデフォルト180px */
  size?: number;
...
export default function ItemCard({
  ...
  const visualBox = (
    <div className={css({ position: "relative", display: "flex", alignItems: "center", justifyContent: "center" })} style={{ width: size, height: size }}>
      {visual}
      {badge && (
        <span
  ...
  const labelEl = (
    <div
      ...
      className={css({ fontSize: "26px", fontWeight: "bold", mt: "10px", lineHeight: 1.25, textAlign: "center" })}
    >
      {label}
    </div>
  );
  ...
  if (!onClick) {
    // 非インタラクティブ表示（持ち物一覧・レシピ帳など）：画像枠だけに枠線をつけ、名前は枠外のキャプションにする
    return (
      <div className={css({ display: "flex", flexDirection: "column", alignItems: "center", width: `${size}px` })}>
        <div
          style={{ background, borderColor }}
          className={css({ border: "2px solid", borderRadius: "12px" })}
        >
          {visualBox}
        </div>
        {labelEl}
  ...
  // インタラクティブ表示（レシピ選択など）：画像・名前・価格/Lvまで含めて縦長の1つのボタンにし、
  // レベルや価格の上をタップしても選択できるようにする。
  return (
    <button
      type="button"
      onClick={onClick}
      ...
      className={css({
        ...
        _hover: { transform: "translateY(-6px) scale(1.04)", boxShadow: "0 12px 32px rgba(0,0,0,0.5)" },
      })}
    >
      {visualBox}
      {labelEl}
